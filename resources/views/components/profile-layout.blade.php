...
<x-layout :title="$title ?? ($pageTitle ?? 'Profil')">
    <div class="profile-container">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        <div class="flex-grow-1">
                            <h2 class="h6 mb-1">Menu Profil</h2>
                            <p class="text-muted small mb-0">Kelola akun dan pengaturan.</p>
                        </div>
                    </div>
        ...
        <div class="col-lg-6">
            <div class="mb-3">
                <h1 class="h5 mb-1">{{ $pageTitle ?? 'Profil' }}</h1>
                <p class="text-muted small mb-0">{{ $attributes->get('subtitle', 'Sesuaikan informasi akun anda.') }}</p>
            </div>
            {{ $slot }}
        </div>
        ...
        <div class="col-lg-3">
            <div class="card border-0 shadow-sm rounded-4 mb-3">
                <div class="card-body">
                    <h2 class="h6 mb-3">Ringkasan Order</h2>
                    ...
                    <div class="d-flex justify-content-between small mb-2">
                        <span class="text-muted">Nilai Pending</span>
                        <span class="fw-semibold">Rp {{ number_format($pendingAmount, 0, ',', '.') }}</span>
                    </div>
                    <div class="d-flex justify-content-between small">
                        <span class="text-muted">Order Terakhir</span>
                        <span class="fw-semibold">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-3">
                <div class="card-body">
                    <h2 class="h6 mb-1">Metode Pembayaran</h2>
                    <p class="text-muted small mb-3">Informasi rekening/kartu tersimpan.</p>
                    ...
                    @if ($accountName || $cardNumber || $cardExpiry || $cardCvc)
                        <div class="mb-2">
                            <div class="text-muted small">Nama Pemegang</div>
                            <div class="fw-semibold">{{ $accountName ?? 'Belum diisi' }}</div>
                        </div>
                        <div class="mb-0">
                            <div class="text-muted small">Nomor Kartu</div>
                            <div class="fw-semibold">{{ $maskedNumber ?? 'Belum diisi' }}</div>
                        </div>
                        ...
                        <div class="alert alert-light border-0 text-muted small mt-3 mb-0">
                            Untuk mengubah data, buka halaman Informasi Pembayaran.
                        </div>
                    @else
                        <div class="alert alert-light border text-muted small mb-0">
                            Belum ada informasi pembayaran. Isi melalui halaman Ubah Profil.
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted small mb-0">{{ $wishlistCount }} item</p>
                    </div>
                    <a href="{{ route('wishlist.index') }}" class="text-decoration-none small">Lihat</a>
                </div>
                <div class="card-body p-0">
                    ...
                    @empty
                        <p class="text-muted small px-3 py-4 mb-0">Belum ada item di wishlist.</p>
                    @endforelse

                    @if ($wishlistCount > $wishlistVisible)
                        <div class="px-3 py-2 text-muted small">
                            +{{ $wishlistCount - $wishlistVisible }} produk lainnya
                        </div>
                    @endif
                </div>
            </div>
